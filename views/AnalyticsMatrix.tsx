...
const Pagination = ({ total, page, setPage, perPage = 10 }: any) => {
  ...
  return (
    <div className="flex justify-between items-center mt-6 bg-white p-3 rounded-xl border border-slate-100 shadow-sm overflow-x-auto">
      <button disabled={page === 1} onClick={() => setPage(page - 1)} className="px-4 py-2 text-xs sm:text-sm font-bold text-slate-600 disabled:opacity-30 hover:bg-slate-100 rounded-lg whitespace-nowrap">← Prev</button>
      <span className="text-xs sm:text-sm font-bold text-slate-400 tracking-widest uppercase mx-4 whitespace-nowrap">Pg <span className="text-slate-800">{page}</span> of {maxPages}</span>
      <button disabled={page === maxPages} onClick={() => setPage(page + 1)} className="px-4 py-2 text-xs sm:text-sm font-bold text-slate-600 disabled:opacity-30 hover:bg-slate-100 rounded-lg whitespace-nowrap">Next →</button>
    </div>
  );
};

// 👈 NEW: Accepts the 4 new time props
export default function AnalyticsMatrix({ generalMatrixData, matrixData, matrixYear, setMatrixYear, matrixMonth, setMatrixMonth, onPhotoClick }: any) {
  ...
  const exportMatrixToCSV = () => {
    ...
    let dataToExport = dashboardTab === 'general' 
      ? matrixData.filter((row: any) => selectedCampaigns.includes('All') || selectedCampaigns.includes(row.campaign))
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500 flex flex-col h-full px-4 sm:px-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4">
        ...
        <div className="flex w-full md:w-auto justify-between md:justify-end gap-4 items-center bg-white md:bg-transparent p-4 md:p-0 rounded-2xl border border-slate-100 md:border-none shadow-sm md:shadow-none">
          ...
          <button onClick={exportMatrixToCSV} className="bg-slate-50 md:bg-white border border-slate-200 text-slate-700 px-4 py-2 md:px-5 md:py-3 rounded-xl text-xs md:text-sm font-bold hover:bg-slate-100 transition-all shadow-sm">⬇️ CSV</button>
        </div>
      </div>

      {/* HORIZONTAL CAMPAIGN TABS (DYNAMIC) */}
      <div className="flex overflow-x-auto space-x-2 bg-white p-1.5 rounded-2xl mb-4 w-full border border-slate-100 shadow-sm scrollbar-hide">
        <button onClick={() => { setDashboardTab('general'); setMatrixPage(1); }} className={`whitespace-nowrap px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-xs md:text-sm font-bold transition-all ${dashboardTab === 'general' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>Executive Summary</button>
        {generalMatrixData.map((camp: any) => (
          <button key={camp.id} onClick={() => { setDashboardTab(camp.campaign); setMatrixPage(1); }} className={`whitespace-nowrap px-4 md:px-6 py-2.5 md:py-3 rounded-xl text-xs md:text-sm font-bold transition-all ${dashboardTab === camp.campaign ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}>
            {camp.campaign}
          </button>
        ))}
      </div>
      ...
      {/* GLOBAL FILTERS ROW */}
      <div className="flex flex-col sm:flex-row gap-3 mb-6 relative z-20 w-full">
        ...
        <div className="bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm flex items-center pr-2 w-full sm:w-auto">
           ...
           <select value={matrixYear} onChange={(e) => setMatrixYear(e.target.value)} className="w-full sm:w-auto border-none bg-slate-50 text-slate-700 text-xs md:text-sm font-bold rounded-xl px-3 py-2 outline-none cursor-pointer mr-1 hover:bg-slate-100 transition-colors">
              {years.map(y => <option key={y} value={y}>{y}</option>)}
           </select>
           <select value={matrixMonth} onChange={(e) => setMatrixMonth(e.target.value)} className="w-full sm:w-auto border-none bg-slate-50 text-slate-700 text-xs md:text-sm font-bold rounded-xl px-3 py-2 outline-none cursor-pointer hover:bg-slate-100 transition-colors">
             ...
             <div className="relative w-full sm:w-auto">
               ...
               {isCampMenuOpen && (
                 <>
                   ...
                   <div className="absolute top-full mt-2 left-0 w-full md:w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 z-50 max-h-72 overflow-y-auto">
      ...
      {/* DATA TABLES */}
      <div className="bg-white border border-slate-100 rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl shadow-slate-200/40 z-10">
          ...
          {dashboardTab === 'general' ? (
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-100 text-[10px] text-slate-400 uppercase tracking-widest">
                  <th className="p-4 sm:p-5 font-bold">Active Campaign</th>
                  <th className="p-4 sm:p-5 font-bold text-center">Stores</th>
                  <th className="p-4 sm:p-5 font-bold text-center w-24 sm:w-28">W1 (1-7)</th>
                  <th className="p-4 sm:p-5 font-bold text-center w-24 sm:w-28">W2 (8-14)</th>
                  <th className="p-4 sm:p-5 font-bold text-center w-24 sm:w-28">W3 (15-21)</th>
                  <th className="p-4 sm:p-5 font-bold text-center w-24 sm:w-28">W4 (22+)</th>
                  <th className="p-4 sm:p-5 font-bold text-right">Liability</th>
                </tr>
              </thead>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
               <thead className="bg-slate-50/80 border-b border-slate-100 text-[10px] text-slate-400 uppercase tracking-widest">
                  <tr>
                    <th className="p-4 sm:p-5 font-bold">Store</th>
                    <th className="p-4 sm:p-5 font-bold text-center">W1 Status</th>
                    <th className="p-4 sm:p-5 font-bold text-center">W2 Status</th>
                    <th className="p-4 sm:p-5 font-bold text-center">W3 Status</th>
                    <th className="p-4 sm:p-5 font-bold text-center">W4 Status</th>
                    <th className="p-4 sm:p-5 font-bold text-right">Payout</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 text-xs sm:text-sm">
                  {paginatedMatrix.map((row: any, i: number) => (
                     <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                       ...
                       <td className="p-4 sm:p-5 text-center">
                          <span onClick={() => onPhotoClick(row.w1)} className={`cursor-pointer px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider ${row.w1.status === 'Approved' ? 'bg-green-100 text-green-700' : row.w1.status === 'Rejected' ? 'bg-red-100 text-red-700' : row.w1.status === 'Missed' ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-700'}`}>{row.w1.status}</span>
                       </td>
                       <td className="p-4 sm:p-5 text-center">
                          <span onClick={() => onPhotoClick(row.w2)} className={`cursor-pointer px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider ${row.w2.status === 'Approved' ? 'bg-green-100 text-green-700' : row.w2.status === 'Rejected' ? 'bg-red-100 text-red-700' : row.w2.status === 'Missed' ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-700'}`}>{row.w2.status}</span>
                       </td>
                       <td className="p-4 sm:p-5 text-center">
                          <span onClick={() => onPhotoClick(row.w3)} className={`cursor-pointer px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider ${row.w3.status === 'Approved' ? 'bg-green-100 text-green-700' : row.w3.status === 'Rejected' ? 'bg-red-100 text-red-700' : row.w3.status === 'Missed' ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-700'}`}>{row.w3.status}</span>
                       </td>
                       <td className="p-4 sm:p-5 text-center">
                          <span onClick={() => onPhotoClick(row.w4)} className={`cursor-pointer px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider ${row.w4.status === 'Approved' ? 'bg-green-100 text-green-700' : row.w4.status === 'Rejected' ? 'bg-red-100 text-red-700' : row.w4.status === 'Missed' ? 'bg-slate-100 text-slate-400' : 'bg-amber-100 text-amber-700'}`}>{row.w4.status}</span>
                       </td>
                       <td className="p-4 sm:p-5 text-right font-black text-green-600">₹{row.totalPayout}</td>
                     </tr>
                  ))}
                </tbody>
            </table>
        ...
        <div className="p-4 sm:p-5 bg-slate-50/50 border-t border-slate-100">
          <Pagination total={dashboardTab === 'general' ? filteredGeneralMatrixData.length : filteredDashboardData.length} page={matrixPage} setPage={setMatrixPage} />
        </div>
      </div>
    </div>
  );
