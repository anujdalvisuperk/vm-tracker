...
export default function MasterSettings({ storesList, campaignsList, reasonsList, fetchData }: any) {
  ...
  const handleDeleteStore = async (id: number, storeName: string) => {
    if (confirm(`Permanently delete ${storeName}?`)) {
      await supabase.from('stores').delete().eq('id', id); fetchData();
    }
  };
  ...
  const handleDeleteCampaign = async (id: number, campName: string) => {
    if (confirm(`Permanently delete campaign "${campName}"?`)) {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2 font-medium text-lg">Manage Store Roster, Campaigns, and Rejection Reasons.</p>
      </div>
      ...
      {settingsTab === 'stores' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-center transition-all hover:shadow-md">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Total Stores</p>
              <p className="text-4xl font-black text-slate-800">{storesList.length}</p>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-center transition-all hover:shadow-md">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Aligned</p>
              <p className="text-4xl font-black text-green-500">{alignedStoresCount}</p>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-center transition-all hover:shadow-md">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Unaligned</p>
              <p className="text-4xl font-black text-amber-500">{storesList.length - alignedStoresCount}</p>
            </div>
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm text-center transition-all hover:shadow-md">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Alignment %</p>
              <p className="text-4xl font-black text-blue-500">{storesList.length ? Math.round((alignedStoresCount / storesList.length) * 100) : 0}%</p>
            </div>
          </div>

          <div className="bg-white p-6 border border-slate-100 rounded-3xl shadow-sm flex flex-col md:flex-row gap-6 items-end">
            ...
            <div>
              ...
              <button onClick={() => fileInputRef.current?.click()} className="border-2 border-slate-200 bg-slate-50 text-slate-700 px-6 py-3 rounded-xl text-sm font-bold hover:bg-slate-100 hover:border-slate-300 flex items-center gap-2 transition-all active:scale-95">
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50">
                <tr className="border-b border-slate-100 text-[10px] text-slate-400 uppercase tracking-widest">
                  <th className="p-5 font-bold w-1/2">Store Name</th>
                ...
                {paginatedStores.map((store: any) => (
                  <tr key={store.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-5 text-right flex justify-end gap-2">
                      {editingStoreId === store.id ? (
                        <button onClick={() => saveEditStore(store.id, store.name)} className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider bg-blue-600 text-white hover:bg-blue-700 transition-all shadow-sm">💾 Save</button>
                      ) : (
                        <button onClick={() => { setEditingStoreId(store.id); setEditingStoreName(store.name); }} className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100 transition-all">✏️ Edit</button>
                      )}
                      <button onClick={() => toggleStoreAlignment(store.id, store.aligned)} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all border ${store.aligned ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100'}`}>
                        {store.aligned ? '✓ Aligned' : '✕ Unaligned'}
                      </button>
                      <button onClick={() => handleDeleteStore(store.id, store.name)} className="px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition-all">🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {settingsTab === 'campaigns' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-1 bg-white p-8 border border-slate-100 rounded-3xl shadow-sm h-fit">
            <h3 className="text-xl font-black text-slate-900 mb-6 border-b border-slate-100 pb-4">Create Campaign</h3>
            <div className="space-y-5">
              <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Campaign Name</label><input type="text" value={newCampName} onChange={(e) => setNewCampName(e.target.value)} className="w-full border-2 border-slate-100 rounded-xl px-4 py-3 text-sm focus:border-blue-400 focus:ring-4 focus:ring-blue-50 outline-none transition-all" /></div>
              <div><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block">Weekly Payout (₹)</label><input type="number" value={newCampPayout} onChange={(e) => setNewCampPayout(e.target.value === '' ? '' : Number(e.target.value))} className="w-full border-2 border-slate-100 rounded-xl px-4 py-3 text-sm focus:border-blue-400 focus:ring-4 focus:ring-blue-50 outline-none transition-all" /></div>
            ...
            {paginatedCampaigns.map((campaign: any) => (
              <div key={campaign.id} className="bg-white border border-slate-100 rounded-3xl p-8 shadow-sm flex flex-col justify-between relative overflow-hidden group hover:shadow-md transition-all">
                ...
                <div className="pt-6 border-t border-slate-100 flex justify-end gap-3">
                  <button onClick={() => setEditingCampaign(campaign)} className="px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider bg-slate-50 text-slate-600 border border-slate-200 hover:bg-slate-100 transition-all flex items-center gap-2">✏️ Edit Setup</button>
                  <button onClick={() => handleDeleteCampaign(campaign.id, campaign.name)} className="px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 transition-all flex items-center gap-2">🗑️ Delete</button>
                </div>
              </div>
            ))}
      ...
      {settingsTab === 'reasons' && (
        <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-sm max-w-3xl">
           <div className="p-8 border-b border-slate-100 bg-slate-50/50">
              ...
              <p className="text-sm text-slate-500 mb-6 font-medium">Add reasons that admins can quickly choose from during reviews.</p>
              <div className="flex gap-3">
                <input type="text" value={newReason} onChange={(e) => setNewReason(e.target.value)} placeholder="e.g. Blurry photo, Not clearly visible..." className="flex-1 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm focus:border-red-400 focus:ring-4 focus:ring-red-50 outline-none transition-all" />
      ...
      {/* --- EDIT CAMPAIGN MODAL (Self Contained) --- */}
      {editingCampaign && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in">
           <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh] overflow-hidden border border-slate-100">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
              <h3 className="font-black text-xl text-slate-900 flex items-center gap-2">✏️ Edit Campaign</h3>
            ...
            <div className="p-8 overflow-y-auto space-y-6">
               ...
               <div className="pt-6 border-t border-slate-100">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 block">🔗 Required Co-Campaigns</label>
               ...
               <div className="pt-6 border-t border-slate-100">
                  <div className="flex justify-between items-center mb-3">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Assigned Stores</label>
                  </div>
